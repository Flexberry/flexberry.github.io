...
toc: true
permalink: ru/audit-data-storage.html
...
** `ObjectPrimaryKey` – [первичный ключ объекта](primary-keys-objects.html), над которым была выполнена аудируемая операция.
...
* [Мастера](master--association.html) находятся на том же уровне, что и собственные поля. 
...
* [Детейлы](detail-associations-and-their-properties.html) отображаются аналогично мастерам, только имена полей задаются по типу <ИмяКласса(номер)> (аналогично было сделано в [старом аудите](audit.html)).
