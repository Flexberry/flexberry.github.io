...
(((
<msg type=note>Чтобы использовать указанную выше запись в шаблоне, требуется, чтобы в контроллере шаблона были определены действия (action) с именами "sortByColumn" и "addColumnToSorting".
Синтаксис "addColumnToSorting=(action "addColumnToSorting")" определяет, что используется ember closure action.</msg>
)))
