...
* Рекомендуется унаследовать все роуты форм редактирования от общего предка, где определено свойство `userName` (это свойство сейчас используется для определения имени текущего пользователя для блокировок).На текущий момент также возможно устанавливать свойство `userName` в роуте каждой отдельной формы редактирования.
