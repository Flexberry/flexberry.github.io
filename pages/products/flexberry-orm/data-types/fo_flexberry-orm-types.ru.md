...
Начиная с .NET 6, Flexberry ORM поддерживает типы `DateOnly` и `TimeOnly`:

* `DateOnly` - представляет дату без времени (год, месяц, день)
* `TimeOnly` - представляет время без даты (час, минута, секунда)

Поддержка этих типов реализована во всех data services (PostgreSQL, Oracle, MSSQL) и LINQ-провайдере. Подробнее в документации вашего СУБД.

## Поддержка .NET 10

Flexberry ORM поддерживает .NET 10 в качестве целевого фреймворка. Для .NET 10 используются следующие зависимости:

| Зависимость | Версия | Описание |
|-------------|--------|----------|
| Microsoft.Data.SqlClient | 6.1.3 | Новый клиент Microsoft для SQL Server |
| Npgsql | 10.0.0 | Провайдер PostgreSQL |
| System.Runtime.Serialization.Formatters | 10.0.0 | Для поддержки BinaryFormatter |

> **Примечание:** Поддержка .NET Core 3.1 удалена из всех проектов.
