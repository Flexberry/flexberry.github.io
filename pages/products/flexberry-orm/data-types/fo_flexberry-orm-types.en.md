...
Starting from .NET 6, Flexberry ORM supports `DateOnly` and `TimeOnly` types:

* `DateOnly` - represents a date without time (year, month, day)
* `TimeOnly` - represents a time without date (hour, minute, second)

These types are supported in all data services (PostgreSQL, Oracle, MSSQL) and LINQ provider. For more details, see the documentation for your database service.

## .NET 10 support

Flexberry ORM supports .NET 10 as a target framework. The following dependencies are used for .NET 10:

| Dependency | Version | Description |
|------------|---------|-------------|
| Microsoft.Data.SqlClient | 6.1.3 | New Microsoft SQL client |
| Npgsql | 10.0.0 | PostgreSQL provider |
| System.Runtime.Serialization.Formatters | 10.0.0 | For BinaryFormatter support |

> **Note:** .NET Core 3.1 support has been removed from all projects.
