...
## TODO: скриншот галочки "нехранимый" в веб-дизайнере
...
В результате вычислимое свойство должно быть:
...
2. Иметь логику вычисления:
   ...
   3. В `computed` свойстве (в приложениях `ember-flexberry` - см. [Вычислимые свойства и проекции моделей](gpg_computable-properties-and-projections-of-models.html))
...
- указывать `DSE` необязательно, но это будет влиять на производительность (см. [раздел](#поведение-сервиса-данных-при-работе-с-вычислимым-полем)) _в некоторых случаях при отсутствии `DSE` поле не может быть вычислено_
...
> Стоит отметить, что в ограничениях по значению перечисления, необходимо указывать заголовок значения. Например, в C# имеется значение `"ОплаченоПокупателем"`, и заголовок `"Оплачено покупателем"`. В ограничении необходимо использовать заголовок `"Оплачено покупателем"`.
...
{% include warning.html content="При изменении детейлов необходимо пересчитывать кэшированные значения для сохранения актуальности данных. Это позволяет сделать [SQL-запрос](fo_sql-query.html), обновляющий кэш при изменениях." %}
...
В связи в архитектурными особенностями [генератора sql-запросов](fo_sql-query.html) во Flexberry ORM, использование вычислимых полей в вычислимых полях __не поддерживается__. При необходимости необходимо скопировать логику вычисления нужного поля:
