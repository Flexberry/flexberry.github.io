...
lang: en 
autotranslated: true 
hash: 9eca534d22f3719ddc438c8e09826bded2e8ac7bc3aebfd5745598800f102e50 
...
Where "componentName" is the name components specified in `flexberry-objectlistview`. 
...
* `columnWidths` - width columns. Using the properties `fixed` you can prevent changing the width of individual columns. If you want to forbid to resize the column or column tools menu, as "columnName", you need to specify `OlvRowToolbar` or `OlvRowMenu` respectively. Formats: 
...
When you copy settings from "Show setup" string constants are enclosed in double quotation marks ("). Because the current standard requires JSCS with the description of object of framing of the string constants in single quotation marks ('), Copy/Paste more convenient the second option descriptions properties "componentName" string. 
