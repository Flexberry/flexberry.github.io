...
To add a component to the ember app, you need to verify the correctness of the configuration [models](efd3_model.html), the serializer and the template.
...
[Model](efd3_model.html) `Подготовка` should contain a reference to the master. [View](TODO) to display the form where it will be located LookUp should also include a description of the wizard.
...
In the [models](efd3_model.html) corresponding to the master type `Пользователь`, you must specify a [view](TODO) under which you will be displaying.
...
In [serializer](efd3_serializer.html) `Подготовка` to describe the link to the wizard:
...
[Serializer](efd3_serializer.html) for the type `Пользователь`:
...
For the list in a LookUp-e you can use the user settings from the [service user settings](TODO).
...
Setting limits on the values displayed in the list for selection, carried out by setting the filter using the [customer query language](efd3_query-language.html). This filter is defined as a function in the property `lookupLimitPredicate` when specifying the template.
...
The service allows you to subscribe to the following events:

Name | Description
:---------------------|:------------------------------------------------------------
`lookupDialogOnShow` | Called when a modal starts to show.
`lookupDialogOnVisible` | Called after a modal has finished showing.
`lookupDialogOnHidden` | Called after a modal has finished hiding.
`lookupOnChange` | Called when lookup value changed.
`lookupDialogOnDataLoaded` | Called when data for lookup are loaded.

An example demonstrating how you can subscribe to an event:
...
In order for the lookup to function in the dropdown mode, you need to set the 'dropdown' property to 'true'. There are also some settings for this mode:

Name | Description | Default value
:---------------------|:------------------------------------------------------------|:----------------
`dropdown` | Flag to show that lookup is in dropdown mode. | false
`dropdownIsSearch` | Flag to show that lookup has search or autocomplete in dropdown mode. | false
`dropdownSettings` | Semantic-ui settings for dropdown. | undefined
`dropdownClass` | CSS classes for flexberry-lookup at dropdown mode. | undefined

You can define values for properties in a template. For example:
...
The implementation is displayed on [ember-stand](http://flexberry.github.io/ember-flexberry/dummy/develop/#/components-examples/flexberry-lookup/dropdown-mode-example).

## Setting the default sort

The implementation is displayed on [ember-stand](http://flexberry.github.io/ember-flexberry/dummy/develop/#/components-examples/flexberry-lookup/default-ordering-example).
