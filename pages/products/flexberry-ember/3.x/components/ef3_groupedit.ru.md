...
`modelLastUpdatedDetail` | Последний обновленный детейл. Инициализируется на форме детейла после инициирования возврата на форму агрегатора. Используется для анализа состояния обновления детейла.
...
Для создания пользовательских кнопок для панели управления groupedit нужно:

В шаблоне для компоненты groupedit задать значение свойства `customButtons` и события для создаваемых кнопок.
