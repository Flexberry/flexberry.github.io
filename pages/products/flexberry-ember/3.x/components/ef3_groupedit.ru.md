...
`flexberry-groupedit` – это компонент Ember.js, используемый в приложениях на базе [платформы Flexberry](ccskrf) для работы с [детейлами](fo_detail-associations-properties.html) на форме редактирования. Этот компонент предоставляет возможность создания, редактирования, удаления записей и поддерживает продвинутые возможности, такие как изменение размера столбцов, пользовательские кнопки и действия для строк.
...
Полный список функций и возможностей, которые предоставляет компонент.
...
`showCheckBoxInRow`| Флаг, определяющий, показывать ли чекбокс в первой колонке каждой строки. Значение по умолчанию: true.
`showDeleteButtonInRow`| Флаг, определяющий, отображать кнопку ли `-` для удаления записи в браузерном и мобильном шаблоне. Значение по умолчанию: false.
...
immediateDelete=immediateDelete| Флаг, определяющий, следует ли немедленно отправлять DELETE-запрос на сервер (для каждой удаленной записи) или нет.
...
`editFormRoute`| Маршрут формы редактирования, который будет открыт после события клика по строке.

**Дополнительные свойства:**

Свойство | Краткое описание
:--------|:----------------
`editOnSeparateRoute` | Флаг, определяющий, следует ли редактировать записи на отдельном маршруте. Значение по умолчанию: false.
...
`saveBeforeRouteLeave` | Необходимо ли сохранять текущую модель перед переходом на маршрут детализации. Значение по умолчанию: false.
...
* Flexberry Groupedit состоит из двух компонентов: `GroupeditToolbar` и `ObjectListView`.
...
DetailInterationService - это сервис, расширяющий стандартный сервис фреймворка Ember и предоставляющий возможность управления управления переходами между формами агрегатора и детейла.
...
`modelCurrentNotSaved` | Текущая не сохраненная модель. Инициализируется на форме детали после инициирования возврата на форму агрегатора. Используется только на форме агрегатора для получения уже созданного, но не сохраненного объекта агрегатора, исключая создание нового экземпляра.
...
`saveBeforeRouteLeave` | Флаг, указывающий на необходимость сохранения текущей модели перед переходом на маршрут агрегатора.
