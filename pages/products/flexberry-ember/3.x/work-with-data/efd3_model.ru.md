...
Модели в ember-приложениях наследуются от класса Ember Data [DS.Model](http://emberjs.com/api/data/classes/DS.Model.html). В "чистом" Ember-приложении объявление модели выглядит следующим образом (для генерации новой модели можно воспользоваться командой `ember generate model test-model-name`, тогда соответствующая модель будет расположена в файле `test-model-name.js` в папке `models`):
...
Получить тип ключа можно через метод [`getMeta` утилиты `information`](efd3_information.html) из аддона `ember-flexberry-data`.
...
Для получения метаданных модели можно воспользоваться методами утилиты [Information](efd3_information.html).

## Вспомогательные модели Flexberry Ember
[Вспомогательные модели аддона `ember-flexberry-data`](https://github.com/Flexberry/ember-flexberry-data/tree/develop/addon/models) включают:

* модели для аудита. Структура хранения данных аудита аналогична [используемой во Flexberry ASP.Net](fa_audit-web.html). В настоящее время ведётся разработка нового варианта аудита для Flexberry Ember.
* [модели для полномочий](efs_security_schema.html).

[Вспомогательные модели аддона `ember-flexberry`](https://github.com/Flexberry/ember-flexberry/tree/develop/addon/models) включают:

* `flexberry-adv-limit` - модель для [задания ограничений](efd3_query-language.html).
* `i-i-s-caseberry-logging-objects-application-log` - модель для [сервиса логирования](efd3_log-service.html).
* `new-platform-flexberry-flexberry-user-setting` - модель для [сервиса пользовательских настроек](efd3_user-setting-service.html).
* `new-platform-flexberry-services-lock` - модель для [сервиса пессимистических блокировок](efd3_lock-service.html).
