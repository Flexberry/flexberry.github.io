...
1. Инициализировать Liquibase в корневой папке проекта: `liquibase init project` (следовать инструкциям на экране). При создании проекта требуется указать адрес БД в формате JDBC URL (например, `jdbc:postgresql://localhost:5432/my-app-db`) и данные для подключения (логин и пароль).
...
**Важно!** "Из коробки" откат изменений работать не будет. Для каждого скрипта необходимо указывать команды для отката (см. [rollback actions](https://docs.liquibase.com/concepts/changelogs/sql-format.html#rollback-actions)). Flexberry Designer на текущий момент не генерирует таких команд.

### Использование скриптов в нескольких БД

В приложении может потребоваться использовать несколько баз данных, и развёртывать скрипты на разных БД (или разных схемах одной БД). Для этого можно воспользоваться контекстами в Liquibase.

*Контекст* - это группа, в которую помещаются changelog'и для того, чтобы фильтровать скрипты по контексту и применять только нужные изменения ([подробнее](https://docs.liquibase.com/concepts/changelogs/attributes/contexts.html)). Можно применить скрипты из контекста на определённой БД.
