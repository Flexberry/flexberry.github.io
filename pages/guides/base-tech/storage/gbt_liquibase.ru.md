...
### Установка Liquibase
Liquibase позволяет запускать команды через свой CLI. В качестве альтернативы, можно запускать CLI в виде Docker контейнера (не рекомендуется если у вас Windows).
