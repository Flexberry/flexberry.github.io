...
Если Вы не планируете перезапускать контейнер после его завершения используйте флаг --rm после аргумента run команды запуска контейнера. В это случае контейнер автоматически удалится из списка завершенных контейнеров.

Для просмотра списка всех работающих и завершенных контейнеров используйте флаг -a команды просмотра списка контейнеров:

```sh
# docker ps -a
```

Для удаления завершенного контейнера испольуйте клманду rm:

```sh
# dcoker rm ID_контейнера_или_кго_имя
```

Если Вам необходим автоматический перезапуск контейнера после его (авариного) останова и автоматический запуск контейнера после перезагрузки системы используйте флаг --restart=unless-stopped после аргумента run команды запуска контейнера. Например:
