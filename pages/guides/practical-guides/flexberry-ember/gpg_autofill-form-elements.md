...
Компонент, отоброжаемый в ячейках таблицы компонента `{% raw %}{{flexberry-groupedit}}{% endraw %}`, получает экземпляр модели в качестве параметра `relatedModel`, с его помощью выведем, описанное нами ранее, вычислимое свойство `actualTotalSum`, в шаблоне.
...
После этого, компонент `{% raw %}{{flexberry-groupedit}}{% endraw %}` для отобраения свойства `totalSum` в ячейке таблицы, будет использовать компонент `{% raw %}{{order-item/total-sum}}{% endraw %}`.
