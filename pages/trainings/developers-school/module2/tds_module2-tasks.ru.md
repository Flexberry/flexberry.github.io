...
5. В баклоге проекта в Azure Boards создать два требования (User Story) со следующими названиями:
...
10. В TFVC-репозиторий проекта на Azure DevOps Services добавить файлы из [GitHub-репозитория библиотеки Chance](https://github.com/chancejs/chancejs):
...
11. В Git-репозиторий проекта на Azure DevOps Services добавить файлы из GitHub-репозитория [Issue Tracker Demo](https://github.com/ehaberev/issue-tracker-demo) (можно предварительно склонировать себе репозиторий на локальную машину).
12. Создать "ленту" Azure Artifacts проекта на Azure DevOps Services:
    * Для возможности работы с артифактами в проекте Azure DevOps Services необходимо для пользователя "основного" (Вашего) аккаунта в настройках организации добавить соответствующее расширение.
    * В настройках "ленты" должен быть указан Upstream-источник для доступа к npm-пакетам репозитория https://registry.npmjs.org/ через данную "ленту" (чтобы в приложении иметь возможность устанавливать не только пакеты из "ленты", но и из данного публичного репозитория npm-пакетов).
13. Создать билд, который будет публиковать npm-пакет `chance-js` из TFVC-репозитория проекта на Azure DevOps Services, в "ленту" Azure Artifacts проекта на Azure DevOps Services: 
    ...
    * С помощью созданного билда опубликовать пакет `chance-js` в "ленту" Azure Artifacts проекта на Azure DevOps Services.
14. В приложение из Git-репозитория проекта на Azure DevOps Services добавить зависимость от npm-пакета `chance-js`, который опубликован в "ленте" Azure Artifacts проекта на Azure DevOps Services:
    ...
    * В приложение должен быть установлен пакет `chance-js` из "ленты" Azure Artifacts проекта на Azure DevOps Services.
...
15. Создать билд, который будет собирать приложение из Git-репозитория проекта на Azure DevOps Services в виде, пригодном для публикации на GitHub Pages: 
    * Можно использовать любой подходящий пул агентов.
...
16. Создать релиз, который будет публиковать артифакты ("результаты") билда приложения из закрытого Git-репозитория на GitHub Pages:
    * Создать пустой релиз в Azure Piplines проекта на Azure DevOps Services.
    * В качестве артифакта для релиза выбрать билд приложения из Git-репозитория проекта на Azure DevOps Services.
